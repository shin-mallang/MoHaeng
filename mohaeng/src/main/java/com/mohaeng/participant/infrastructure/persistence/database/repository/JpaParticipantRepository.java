...
public interface JpaParticipantRepository extends JpaRepository<Participant, Long>, ParticipantRepository {
    ...
    @Override
    @Query("select p from Participant p join fetch p.member join fetch p.clubRole where p.clubRole.clubRoleCategory = 'PRESIDENT'")
    ...
    @Override
    @Query("select p from Participant p join fetch p.clubRole join fetch p.club where p.id = :id")
    Optional<Participant> findWithClubAndClubRoleById(@Param("id") final Long id);
}
